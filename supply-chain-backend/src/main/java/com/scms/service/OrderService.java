        ...
        if (order.getVehicleId() != null) {
            ...
            if (vehicle != null) {
                ...
                if (isVehicleFirstTimeAssigned) {
                    ...
                    // NOTE: Dispatch OTP is now generated by DeliveryOtpService.generateDispatchOtp()
                    // called explicitly from the frontend "Ready For Dispatch" button.
                    // Old plaintext OTP removed in favour of BCrypt-hashed OTP in order_delivery_otps table.

                    // Notifications for vehicle assignment
                    try {
                        notificationService.sendNotification(
                            "AI Vehicle Assigned",
                        ...
                        notificationService.sendNotification(
                            "Shipment Ready",
                            "Your order #" + order.getOrderId() + " has been assigned a vehicle and is being prepared for dispatch.",
                            "ORDER",
                            "INFO",
                            order.getOrderId(),
