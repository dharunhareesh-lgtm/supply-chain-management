/**
 * LogisticsApplications.jsx — Dedicated Admin Review Queue for Logistics Provider Applications.
 */
import { useState, useEffect } from "react";
import { useNavigate } from "react-router-dom";
import {
  Truck, Clock, CheckCircle2, XCircle, Eye, RefreshCw, MapPin
} from "lucide-react";
...
function LogisticsApplications() {
  ...
  const fetchData = async () => {
    ...
    try {
      const [reqRes, countRes] = await Promise.all([
        fetch(`${API}/api/admin/partner-requests?role=logistics`),
        fetch(`${API}/api/admin/partner-requests/counts?role=logistics`)
      ]);
      ...
      console.error("Failed to fetch logistics applications", err);
    } finally {
      setLoading(false);
    }
  };
      ...
      <PageShell sidebar={<AdminSidebar />}>
        <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
          <PageHeader
            ...
            subtitle="Review logistics fleet applications, verify transport coverage and fleet capabilities, and approve onboarding credentials."
            action={
          ...
          <DashCard>
            <CardHeader
              title="Logistics Onboarding Queue"
              subtitle="Transport providers & fleet operators awaiting platform clearance"
              icon={Truck}
            />
            ...
            <div style={{ marginTop: "16px" }}>
              <Toolbar
                ...
                placeholder="Search by company name, contact, email, or area..."
                filter={filter}
                onFilter={setFilter}
                filterOptions={[
              ...
              <TableWrap loading={loading}>
                {filteredRequests.length === 0 ? (
                  <EmptyState
                    title="No logistics applications found"
                      ...
                      {filteredRequests.map((r) => (
                        <tr key={r.id}>
                          ...
                          <td>
                            ...
                            <div style={{ fontSize: "12px", color: "var(--ink-soft)" }}>Contact: {r.contactPerson}</div>
                          </td>
                          <td>
                            <div style={{ fontSize: "13px", color: "#a78bfa", fontWeight: 500, maxWidth: "260px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                              {r.description || "Transport Services"}
                            </div>
                            <div style={{ fontSize: "11px", color: "var(--ink-soft)" }}>
                              {r.yearsOfExperience ? `${r.yearsOfExperience} Vehicles in Fleet` : "Fleet Operator"}
                            </div>
                          </td>
                          <td>
          ...
          <div style={{
            ...
            <div style={{
              ...
              <h3 style={{ fontSize: "18px", fontWeight: 700, color: "#fff", marginBottom: "8px" }}>
                {actionModal.type === "approve" ? "Approve Logistics Application" : "Reject Application"}
              </h3>
...
export default LogisticsApplications;
